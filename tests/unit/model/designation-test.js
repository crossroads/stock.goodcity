...
test("Relationships with other models", function(assert) {
  ...
  var relationshipWithStockitContact = Ember.get(
    designation,
    "relationshipsByName"
  ).get("stockitContact");
  var relationshipWithOrganisation = Ember.get(
    designation,
    "relationshipsByName"
  ).get("organisation");
  var relationshipWithLocalOrder = Ember.get(
    designation,
    "relationshipsByName"
  ).get("localOrder");
  var relationshipWithItem = Ember.get(designation, "relationshipsByName").get(
    "items"
  );
  var relationshipOrdersPackage = Ember.get(
    designation,
    "relationshipsByName"
  ).get("ordersPackages");
  var relationshipOrdersPurpose = Ember.get(
    designation,
    "relationshipsByName"
  ).get("ordersPurposes");
...
test("check dispatchedItems returns items with sentOn not null", function(assert) {
  ...
  assert.equal(
    Ember.compare(despatchedItemsIds, [item1.get("id"), item2.get("id")]),
    0
  );
});
...
test("check designatedItems returns items with sentOn null", function(assert) {
  ...
  assert.equal(
    Ember.compare(designatedItemsIds, [item1.get("id"), item2.get("id")]),
    0
  );
});
...
test("check designatedOrdersPackages returns only designated orders_packages", function(assert) {
  ...
  assert.equal(
    Ember.compare(designatedOrdersPackagesIds, [
...
test("check dispatchedOrdersPackages returns only dispatched orders_packages", function(assert) {
  ...
  assert.equal(
    Ember.compare(dispatchedOrdersPackagesIds, [
      orders_package1.get("id"),
      orders_package2.get("id")
    ]),
    0
