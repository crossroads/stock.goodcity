...
test("check attributes", function(assert) {
  ...
  var recentlyUsedAt =
    Object.keys(model.toJSON()).indexOf("recentlyUsedAt") > -1;
...
test("Relationships with other models", function(assert) {
  ...
  var relationshipWithStockitContact = get(
    designation,
    "relationshipsByName"
  ).get("stockitContact");
  var relationshipWithOrganisation = get(
    designation,
    "relationshipsByName"
  ).get("organisation");
  var relationshipWithLocalOrder = get(designation, "relationshipsByName").get(
    "localOrder"
  );
  var relationshipWithItem = get(designation, "relationshipsByName").get(
    "items"
  );
  var relationshipOrdersPackage = get(designation, "relationshipsByName").get(
    "ordersPackages"
  );
  var relationshipOrdersPurpose = get(designation, "relationshipsByName").get(
    "ordersPurposes"
  );
...
test("check dispatchedItems returns items with sentOn not null", function(assert) {
  ...
  assert.equal(
    compare(despatchedItemsIds, [item1.get("id"), item2.get("id")]),
    0
  );
});
...
test("check designatedItems returns items with sentOn null", function(assert) {
  ...
  assert.equal(
    compare(designatedItemsIds, [item1.get("id"), item2.get("id")]),
    0
  );
});
...
test("check designatedOrdersPackages returns only designated orders_packages", function(assert) {
  ...
  run(function() {
    orders_package1 = store.createRecord("orders_package", {
      id: 1,
      state: "designated"
    });
    orders_package2 = store.createRecord("orders_package", {
      id: 2,
      state: "designated"
    });
    orders_package3 = store.createRecord("orders_package", {
      id: 3,
      state: "dispatched"
    });
    model
      .get("ordersPackages")
      .pushObjects([orders_package1, orders_package2, orders_package3]);
  });

  designatedOrdersPackagesIds = model
    .get("designatedOrdersPackages")
    .getEach("id");

  assert.equal(designatedOrdersPackagesIds.get("length"), 2);
  assert.equal(
    compare(designatedOrdersPackagesIds, [
      orders_package1.get("id"),
      orders_package2.get("id")
    ]),
    0
  );
});
...
test("check dispatchedOrdersPackages returns only dispatched orders_packages", function(assert) {
  ...
  dispatchedOrdersPackagesIds = model
    .get("dispatchedOrdersPackages")
    .getEach("id");
  assert.equal(dispatchedOrdersPackagesIds.get("length"), 2);
  assert.equal(
    compare(dispatchedOrdersPackagesIds, [
      orders_package1.get("id"),
      orders_package2.get("id")
    ]),
    0
...
test("check allItemsDispatched returns true if all Items are dispatched otherwise returns false", function(assert) {
  ...
  run(function() {
    ...
    unDispatcheditem = store.createRecord("item", {
      id: 4,
      isDispatched: false
    });
