...
test("Relationships with other models", function(assert) {
  ...
  var relationshipWithItem = get(PackagesLocation, "relationshipsByName").get(
    "item"
  );
  var relationshipWithLocation = get(
    PackagesLocation,
    "relationshipsByName"
  ).get("location");
...
test("check siblingPackagesLocations computed property", function(assert) {
  ...
  run(function() {
    ...
    packagesLocation1 = store.createRecord("packagesLocation", {
      id: 1,
      quantity: 1
    });
    packagesLocation2 = store.createRecord("packagesLocation", {
      id: 2,
      quantity: 2
    });
    item
      .get("packagesLocations")
      .pushObjects([packagesLocation1, packagesLocation2, model]);
  });
  assert.equal(
    compare(model.get("siblingPackagesLocations").getEach("id"), [
      "1",
      "2",
      "3"
    ]),
