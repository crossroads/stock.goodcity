...
test("Relationships with other models", function(assert) {
  ...
  var relationshipWithDesignation = get(Item, "relationshipsByName").get(
    "designation"
  );
  var relationshipWithLocation = get(Item, "relationshipsByName").get(
    "location"
  );
  var relationshipWithCode = get(Item, "relationshipsByName").get("code");
  var relationshipWithDonorCondition = get(Item, "relationshipsByName").get(
    "donorCondition"
  );
  var relationshipWithSetItem = get(Item, "relationshipsByName").get("setItem");
  var relationshipWithPkgsLocation = get(Item, "relationshipsByName").get(
    "packagesLocations"
  );
  var relationshipWithOrdersPkgs = get(Item, "relationshipsByName").get(
    "ordersPackages"
  );
...
test("check validUndispatchedLocations computed property", function(assert) {
  ...
  assert.equal(
    compare(validUndispatchedLocationsIds, [packageLocation1.get("id")]),
    0
  );
});
...
test("check ordersPackagesWithStateDesignatedAndDispatched computed property", function(assert) {
  ...
  assert.equal(
    compare(model.get("ordersPackagesWithStateDesignatedAndDispatched"), [
      ordersPackage1,
      ordersPackage2
    ]),
    0
...
test("check packagesLocationsList computed property", function(assert) {
  ...
  assert.equal(
    compare(model.get("packagesLocationsList"), [
      packageLocation1,
      packageLocation2,
      packageLocation3
    ]),
...
test("check imageUrlList computed property", function(assert) {
  ...
  run(function() {
    ...
    image1 = store.createRecord("image", {
      id: 1,
      cloudinary_id: "1416902230/mmguhm3zdkonc2nynjue.jpg",
      favourite: false,
      itemId: 1
    });
    image2 = store.createRecord("image", {
      id: 2,
      cloudinary_id: "1416902232/mmguhm3zdkonc2nynjue2.jpg",
      favourite: false
    });
  });

  assert.equal(
    compare(item.get("imageUrlList"), [
