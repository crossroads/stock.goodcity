...
module("Acceptance: Order summary", {
  beforeEach: function() {
    ...
    $.mockjax({
      url: "api/v1/beneficiar*" + beneficiary.id,
      type: "DELETE",
      status: 200,
      responseText: {}
    });
...
test("Order summary detail", function(assert) {
  ...
  assert.equal(
    $("#submitted_count")
      .text()
      .trim(),
    designation.get("submitted")
  );
  assert.equal(
    $("#scheduled_count")
      .text()
      .trim(),
    designation.get("dispatching")
  );
  assert.equal(
    $("#cancelled_count")
      .text()
      .trim(),
    designation.get("cancelled")
  );
  assert.equal(
    $("#closed_count")
      .text()
      .trim(),
    designation.get("closed")
  );
});
...
test("Client/Purpose displays beneficiary details", function(assert) {
  ...
  andThen(function() {
    assert.equal(currentPath(), "orders.client_summary");
    assert.equal(
      $(".beneficiary-title").val(),
      designation.get("beneficiary.fullNameWithoutTitle")
    );
    assert.equal(
      $(".beneficiary-phone").val(),
      designation.get("beneficiary.mobileWithoutCountryCode")
    );
    assert.equal(
      $(".beneficiary-identity-type").val(),
      designation.get("beneficiary.identityType.name")
    );
    assert.equal(
      $(".identity-number").val(),
      designation.get("beneficiary.identityNumber")
    );
    assert.equal($(".people-helped").val(), designation.get("peopleHelped"));
    assert.equal(
      $(".order-description-textarea").val(),
      designation.get("purposeDescription")
    );
  });
});
