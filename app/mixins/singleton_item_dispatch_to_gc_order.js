...
export default Ember.Mixin.create({
  ...
  canDispatchOrder(designation) {
    return (
      designation &&
      !designation.get("isDispatching") &&
      designation.get("allDesignatedOrdersPackages")
    );
  },
  ...
  actions: {
    dispatchOrdersPackagePopUp(item, ordersPkgId) {
      if (!item.get("isSingletonItem")) {
        return false;
      }
      var _this = this;
      var designation = item
        .get("ordersPackages")
      ...
      if (this.canDispatchItems(designation)) {
        _this
          .get("messageBox")
          .alert(
            _this.get("intl").t("order_details.complete_process_warning"),
            () => {
              this.set("hidden", true);
            }
          );
        ...
        this.genericCustomPopUp(
          "order_details.first_item_dispatch_warning",
          "item.cap_dispatch",
          "not_now",
          function() {
            _this.send("apiRequests", item, designation, ordersPkgId);
          }
        );
      } else {
    ...
    dispatchItem(item, designation, ordersPkgId) {
      ...
      new AjaxPromise(url, "PUT", this.get("session.authToken"), {
        ...
        .finally(() => {
          ...
          if (
            designation.get("allDispatchedOrdersPackages") &&
            designation.get("isDispatching")
          ) {
    ...
    closeOrderPoUp(designation) {
      var _this = this;
      this.genericCustomPopUp(
        "order_details.close_order_popup",
        "order.close_order",
        "not_now",
        function() {
          _this.send("updateOrderState", designation, "close");
        }
      );
    },
