 ...
 */
export default Ember.Controller.extend(AsyncMixin, EmbedScannerMixin, {
  // ----------------------
  // Dependencies
  // ----------------------
  ...
  actions: {
    ...
    async addItem(pkg, opts = {}) {
      ...
      const revision =
        this.get('revisions').find(rev => Number(rev.get('itemId')) === Number(pkg.get('id'))) ||
    ...
    commit() {
      this.stopScanning();
      this.runTask(() => {
        return this.get('stocktakeService').commitStocktake(this.get('stocktake'));
    ...
    async cancelStocktake() {
      ...
      this.runTask(() => {
        return this.get('stocktakeService').cancelStocktake(this.get('stocktake'));
      }, ERROR_STRATEGIES.MODAL);
    },
