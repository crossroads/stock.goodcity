...
export default orderUserOrganisation.extend({
  ...
  async model() {
    ...
    return hash({
      ...
      orderTransport:
        orderTransport &&
        orderTransport.filterBy("orderId", orderId).get("firstObject")
    });
  },
  /* jshint ignore:end */
  ...
  setUpFormData(model, controller) {
    ...
    if (orderTransport) {
      ...
      if (selectedDate) {
        slots = availableDatesAndTime.appointment_calendar_dates.filter(
          date => date.date === selectedDate.format("YYYY-MM-DD")
        )[0].slots;
