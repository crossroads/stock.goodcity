...
export default Ember.Component.extend({
  ...
  didRender() {
    ...
    let itemStateFilters = JSON.parse(localStorage.getItem("itemStateFilters"));
    if(itemStateFilters && itemStateFilters.length) {
      itemStateFilters = this.removeA(itemStateFilters, "published_and_private", "with_and_without_images");
    }
    let itemLocationFilters = localStorage.getItem("itemLocationFilters");
    this.changeCssAndBtnName(itemLocationFilters, "#item-location-filter", "Location");
  ...
  actions: {
    redirectTofilters(queryParam) {
      ...
      this.get('router').transitionTo("item_filters", {queryParams: itemFilter});
    }
  }
});
